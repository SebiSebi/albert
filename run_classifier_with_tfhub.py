...
def create_tokenizer_from_hub_module(albert_hub_module_handle):
  ...
  with tf.Graph().as_default():
    ...
    with tf.Session() as sess:
      ...
      # The sentencepiece module doesn't know how to read GCS files. Therefore,
      # if the spm model file is not local, copy it to o temporary local file.
      local_vocab_file = vocab_file
      if not os.path.isfile(vocab_file) or not os.path.exists(vocab_file):
        _, local_vocab_file = tempfile.mkstemp()
        tf.logging.info("{} is not local. Copying it to {}".format(vocab_file, local_vocab_file))
        tf.io.gfile.copy(vocab_file, local_vocab_file, overwrite=True)
      tf.logging.info("Using {} sentencepiece model.".format(local_vocab_file))
      tf.logging.info("Lower case: {}".format(do_lower_case))
      FLAGS.spm_model_file = local_vocab_file
...
def main(_):
  ...
  if FLAGS.do_train:
    train_examples = processor.get_train_examples(FLAGS.data_dir)[:100000]
    num_train_steps = int(
